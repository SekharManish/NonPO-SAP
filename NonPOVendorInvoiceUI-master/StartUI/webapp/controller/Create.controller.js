sap.ui.define(["sap/ui/core/mvc/Controller",
    ...
    return BaseController.extend("com.sap.cp.dpa.invwpo.StartUI.controller.Create", {
        ...
        onInit: function () {
            ...
            this.getView().setModel(new JSONModel(), "DOXModel");           
        ...
        formatDownloadUrl: function (objectId) {
            return "/comsapcpdpainvwpoStartUI/docservice/workflowmanagement/NonPOVendorInvoiceDocuments/"
        ...
        onOpenDocXSaaS: function (oEvent) {
            ...
            if (this._getInputVal("docXJobID")) {
                url = "https://itcwfmdev.ui.aiservices-dox.cfapps.sap.hana.ondemand.com/index.html#/invoiceviewer&/iv/detailDetail/" + this._getInputVal("docXJobID") + "/ThreeColumnsMidExpanded";
            }
        ...
        // On Success handling of Document Service
        _handleDOXSuccess: function (oResponse, sMsg, data) {
            if (oResponse.extraction && oResponse.extraction.headerFields.length) {
                this.getView().getModel("createModel").setProperty("/requesterName", "");
                this.getView().getModel("requesterModel").setProperty("/value", "");
                this.getView().getModel("createModel").setProperty("/reference", "");
                this.getView().getModel("createModel").setProperty("/amount", "");
                this.getView().getModel("vendorListModel").setProperty("/value", "");
                this.getView().getModel("vendorListModel").setProperty("/desc", "");
                this.getView().getModel("vendorListModel").setProperty("/error", "None");
                this.getView().getModel("companyListModel").setProperty("/value", "");
                ...
                this.getView().getModel("createModel").setProperty("/invoiceDate", "");
                this.getView().getModel("paymentTermListModel").setProperty("/value", "");
                this.getView().getModel("paymentTermListModel").setProperty("/desc", "");
                this.getView().getModel("createModel").setProperty("/days1", "");
                this.getView().getModel("createModel").setProperty("/discount1", "");
                this.getView().getModel("createModel").setProperty("/days2", "");
                this.getView().getModel("createModel").setProperty("/discount2", "");
                this.getView().getModel("createModel").setProperty("/days3", "");;


                for (var i = 0; i < oResponse.extraction.headerFields.length; i++) {
                    var sUrl, oData, bCleanFlag, sName = oResponse.extraction.headerFields[i].name,
                        sValue = oResponse.extraction.headerFields[i].value;
                    switch (sName) {
                        case "documentNumber":
                            this.getView().getModel("createModel").setProperty("/reference", sValue);
                            break;
                        case "netAmount":
                            this.getView().getModel("createModel").setProperty("/amount", sValue);
                            break;
                        case "receiverContact":
                            if (sValue === "") {
                                this.getView().getModel("createModel").setProperty("/requesterName", sValue);
                                this.getView().getModel("requesterModel").setProperty("/value", sValue);
                            } else if (oResponse.enrichment.employee.length) {
                                var sConfidence = oResponse.enrichment.employee[0].confidence,
                                    sId = oResponse.enrichment.employee[0].id;
                                for (var j = 1; j < oResponse.enrichment.employee.length; j++) {
                                    if (oResponse.enrichment.employee[j].confidence > sConfidence) {
                                        sConfidence = oResponse.enrichment.employee[j].confidence,
                                            sId = oResponse.enrichment.employee[j].id;
                                    }
                                }
                                this.getView().getModel("requesterModel").setProperty("/value", sId);
                                this.getView().getModel("createModel").setProperty("/requesterName", sValue);
                            } else {
                                sUrl = "/comsapcpdpainvwpoStartUI/CPI/http/getUserList";
                                var aFullName = sValue.split(" "), sFirstName, sMidName, sLastName;
                                if (aFullName.length >= 3 && aFullName.length) {
                                    sFirstName = aFullName[0];
                                    sMidName = aFullName[1];
                                    sLastName = aFullName[2];
                                } else if (aFullName.length <= 2 && aFullName.length) {
                                    sFirstName = aFullName[0];
                                ...
                                this.getView().getModel("DOXModel").setProperty("/data1", oData1);
                                this.getView().getModel("DOXModel").setProperty("/data2", oData2);
                                this.getView().getModel("DOXModel").setProperty("/userCallCount", 1);
                                this.getView().getModel("DOXModel").setProperty("/userCallUrl", sUrl);
                                this.getView().getModel("createModel").setProperty("/requesterName", sValue);
                                bCleanFlag = 1;
                            }
                            break;
                        case "senderName":
                            if (oResponse.enrichment.sender.length) {
                                var sConfidence = oResponse.enrichment.sender[0].confidence,
                                    sId = oResponse.enrichment.sender[0].id;
                                for (var k = 1; k < oResponse.enrichment.sender.length; k++) {
                                    if (oResponse.enrichment.sender[k].confidence > sConfidence) {
                                        sConfidence = oResponse.enrichment.sender[k].confidence,
                                            sId = oResponse.enrichment.sender[k].id;
                                    }
                                }
                                this.getView().getModel("vendorListModel").setProperty("/value", sId);
                            } else {
                            ...
                            this.getView().getModel("DOXModel").setProperty("/doxVendorValue", sValue);
                            this.getView().getModel("vendorListModel").setProperty("/desc", sValue);
                            break;
                        case "receiverName":
                            ...
                            oData = JSON.stringify(oData);
                            bCleanFlag = 1;
                            this.getView().getModel("DOXModel").setProperty("/doxCCValue", sValue);
                            break;
                        case "documentDate":
                            this.getView().getModel("createModel").setProperty("/invoiceDate", sValue);
                            this.getView().byId("idInvoiceDate").fireEvent("change");
                    ...
                    if (bCleanFlag && sName === "receiverName") {
                        var that = this;
                        jQuery.ajax({
                            url: sUrl,
                            type: "POST",
                            ...
                            success: jQuery.proxy(this._DOXSuccess, this),
                            error: jQuery.proxy(this._DOXError, this)
                        });
                        bCleanFlag = 0;
                    } else if (bCleanFlag && sName === "receiverContact") {
                        var that = this;
                        jQuery.ajax({
                            url: sUrl,
                            type: "POST",
                            data: oData,
                            contentType: "application/json",
                            success: jQuery.proxy(this._DOXEmpSuccess, this),
                            error: jQuery.proxy(this._DOXError, this)
                        });
                        bCleanFlag = 0;
                    } else if (bCleanFlag) {
                        ...
                        jQuery.ajax({
                            ...
                            data: oData,
                            contentType: "application/json",
                            success: jQuery.proxy(this._DOXVSuccess, this),
                            error: jQuery.proxy(this._DOXError, this)
                        });
                        bCleanFlag = 0;
                    }
            ...
            console.log("success");
            this.getView().getModel("DOXModel").setProperty("/error", "None");
        },

        _DOXEmpSuccess: function (oResponse, data) {
            if (oResponse === "" && this.getView().getModel("DOXModel").getProperty("/userCallCount") === 1) {
                var that = this;
                var oData = this.getView().getModel("DOXModel").getProperty("/data1");
                oData = JSON.stringify(oData);
                jQuery.ajax({
                    url: this.getView().getModel("DOXModel").getProperty("/userCallUrl"),
                    type: "POST",
                    data: oData,
                    contentType: "application/json",
                    success: jQuery.proxy(this._DOXEmpSuccess, this),
            ...
            } else if (oResponse === "" && this.getView().getModel("DOXModel").getProperty("/userCallCount") === 2) {
                var that = this;
                var oData = this.getView().getModel("DOXModel").getProperty("/data2");
                oData = JSON.stringify(oData);
                jQuery.ajax({
                    url: this.getView().getModel("DOXModel").getProperty("/userCallUrl"),
                    type: "POST",
                ...
                this.getView().getModel("requesterModel").setProperty("/value", oResponse.userList.userName);
                this.getView().getModel("requesterModel").setProperty("/error", "None");
                this.getView().getModel("createModel").setProperty("/requesterName", oResponse.userList.fullname);
            }

            if (this.getView().getModel("DOXModel").getProperty("/userCallCount") === 3) {
                this.getView().getModel("DOXModel").setProperty("/userCallCount", 0);
            } else {
                this.getView().getModel("DOXModel").setProperty("/userCallCount", this.getView().getModel("DOXModel").getProperty("/userCallCount") + 1);
            }
        },

        _DOXVSuccess: function (oResponse, data) {
            var bNoMatchFlag;
            if (oResponse.root) {
                if (oResponse.root.Result) {
                    if (oResponse.root.Result[0]) {
                        this.getView().getModel("vendorListModel").setProperty("/value", oResponse.root.Result[0].Vendor);
                    } else {
                        this.getView().getModel("vendorListModel").setProperty("/value", oResponse.root.Result.Vendor);
                    }
                    this.getView().getModel("vendorListModel").setProperty("/error", "None");
                    this.getView().byId("idVendor").fireEvent("submit");
                }
            } else {
                this.getView().getModel("vendorListModel").setProperty("/value", this.getView().getModel("DOXModel").getProperty("/doxVendorValue"));
                this.getView().getModel("vendorListModel").setProperty("/desc", "");
        ...
        _DOXSuccess: function (oResponse, data) {
            var bNoMatchFlag;
            if (oResponse.root) {
                if (oResponse.root.companyList) {
                    for (var i = 0; i < oResponse.root.companyList.length; i++) {
                        if (oResponse.root.companyList[i].description === this.getView().getModel("DOXModel").getProperty("/doxCCValue")) {
                            this.getView().getModel("companyListModel").setProperty("/value", oResponse.root.companyList[i].companyCode);
                            this.getView().getModel("companyListModel").setProperty("/desc", oResponse.root.companyList[i].description);
                            this.getView().getModel("companyListModel").setProperty("/error", "None");
                            bNoMatchFlag = true;
                            this.getView().byId("idCompanyCode").fireEvent("liveChange");
                            this.getView().byId("idCompanyCode").fireEvent("submit");
                        }
                    }
                    //when company code description from document is not exact match of company code description in the backend
                    if (!bNoMatchFlag) {
                        this.getView().getModel("companyListModel").setProperty("/value", this.getView().getModel("DOXModel").getProperty("/doxCCValue"));
                        this.getView().getModel("companyListModel").setProperty("/desc", "");
        ...
        _DOXError: function (oError) {
            console.log("error while data clean call");
        },



        _handleDOXError: function (oError) {
            console.log("error");
            this.getView().getModel("DOXModel").setProperty("/error", "Error");
            this.getView().byId("idDOXDisplay").setEnabled(false);
        },
        // Retriving the data which is been exported from Job ID of Dox Extraction
        onRetrieveExtractedValue: function (oEvent) {
            if (this._getInputVal("docXJobID")) {
                this.getView().byId("idDOXDisplay").setEnabled(true);
                var doxURL = "/comsapcpdpainvwpoStartUI/DOX/v1/document/jobs/" + this._getInputVal("docXJobID") + "?extractedValues=false";
                jQuery.ajax({
                    url: doxURL,
                    type: "GET",
                    contentType: "application/json",
                    success: jQuery.proxy(this._handleDOXSuccess, this),
                    error: jQuery.proxy(this._handleDOXError, this)
                });
            } else {
                this.getView().byId("idDOXDisplay").setEnabled(false);
                this.getView().getModel("DOXModel").setProperty("/error", "None");
        ...
        onAfterRendering: function () {
            ...
            Util.oParamObject.invoiceDetails.headerDetail.initiator = user.getUser().getFullName();
            Util.oParamObject.invoiceDetails.headerDetail.initiatorMailId = user.getUser().getEmail();
        ...
        checkIfFolderExists: function (folderName) {
            ...
            if (folderName == "workflowmanagement") {
                var sUrl = "/comsapcpdpainvwpoStartUI/docservice/";
            } else if (folderName == "NonPOVendorInvoiceDocuments") {
                var sUrl = "/comsapcpdpainvwpoStartUI/docservice/workflowmanagement/";
            } else {
                var sUrl = "/comsapcpdpainvwpoStartUI/docservice/workflowmanagement/NonPOVendorInvoiceDocuments";
            }
            var oSettings = {
                "url": sUrl,
                "method": "GET",
        ...
        _resetModel: function () {
            ...
            this.getView().getModel("DOXModel").setProperty("/value", []);
            this.getView().getModel("DOXModel").setProperty("/error", "None");
        },       
        ...
        triggerTaxCalculation: function (oTax) {
            ...
            jQuery.ajax({
                url: "/comsapcpdpainvwpoStartUI/CPI/http/fetchTaxcalculation",
                type: "POST",
                data: oData,
                contentType: "application/json",
        ...
        handleTaxData: function (sJson) {
            ...
            if (sJson !== "") {
                ...
                if (aMsg && aMsg.length) {                   
                    ...
                    jQuery.ajax({
                        url: "/comsapcpdpainvwpoStartUI/CPI/http/CheckInvoice",
                        type: "POST",
                        data: data,
                        contentType: "application/json",
        ...
        */
        ...
        createFolder: function (folderName) {
            ...
            if (folderName == "workflowmanagement") {
                ...
                var oUploadCollection = this.getView().byId("UploadCollection");
                oUploadCollection.setUploadUrl("/comsapcpdpainvwpoStartUI/docservice/workflowmanagement/NonPOVendorInvoiceDocuments/" + folderName);
                console.log("creating temporary folder with a name '" + folderName + "'");
                var sUrl = "/comsapcpdpainvwpoStartUI/docservice/workflowmanagement/NonPOVendorInvoiceDocuments/";
            }
        ...
        _triggerBackendCallToSaveChanges: function (sToken, oDialog) {
            ...
            var oController = this;
            $.ajax({
                url: "/comsapcpdpainvwpoStartUI/workflowruntime/v1/workflow-instances",
                method: "POST",
                async: false,
                contentType: "application/json",
                headers: {
       ...
       // Success handler for Duplicate Invoice check      
        ...
        _handleDuplicateInvoiceFetch: function () {
            ...
            that.duplicateOut = [];
            jQuery.ajax({
                url: "/comsapcpdpainvwpoStartUI/CPI/http/getBSIK",
                type: "POST",
                data: oData1,
                contentType: "application/json",
        ...
        setParameters: function (oDialog) {
            ...
            oParam.invoiceDetails.headerDetail.initiator = user.getUser().getFullName();
            oParam.invoiceDetails.headerDetail.initiatorMailId = user.getUser().getEmail();
        ...
        _populatePaymentTerm: function () {
            if (this.vendorflag && this.deadlockflag || this.companycodeflag && this.deadlockflag || this.invoicedateflag && this.deadlockflag) {
                ...
                if (sCC && sVendor) {
                    jQuery.ajax({
                        url: "/comsapcpdpainvwpoStartUI/CPI/http/getVendorDetails",
                        type: "POST",
                        data: oData,
                        contentType: "application/json",
