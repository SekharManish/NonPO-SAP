sap.ui.define(["sap/ui/core/mvc/Controller",
    ...
    return BaseController.extend("com.sap.cp.dpa.invwpo.ReworkUI.controller.AddItem", {
        ...
        onAfterRendering: function () {
            ...
            if (Util.storeEditItemProperty && Util.editItem) {
        ...
        handleAddItemDetails: function (oEvent) {
            ...
            if (this.getView().getModel("GLAccountModel").getProperty("/value") && sAmount) {
                ...
                if (Util.sPath && Util.createView.getModel('ItemDetails')) {
                    ...
                    Util.createView.getModel('ItemDetails').refresh(true);
        ...
        onFieldValidation: function (oEvent) {
            var sUrl = "/comsapcpdpainvwpoStartUI/CPI/http/getInternalOrder";
            this.sInputValue = oEvent.getSource().getValue();
            var sSearchString = "%" + this.sInputValue + "%";
            var sCompanyCode = this.getView("Create").getModel("companyListModel").getProperty("/value");
            var oData = {
        ...
        handleFieldValidationSuccess: function (oResponse, data) {
            if(oResponse){
                if(oResponse.internalOrderList){
                    if(oResponse.internalOrderList.internalOrder !== "" && oResponse.internalOrderList.internalOrder === this.sInputValue && oResponse.internalOrderList.internalOrder){
